...
- 已完成路由或接口：
  - `/health` —— 健康检查（无需登录）
  - `/api/metrics` —— 指标卡 JSON API（TODO 8-1）
  - `/api/categories?category=...` —— 品类筛选 JSON API（TODO 8-2）
  - 统一 400 错误 JSON 响应（TODO 8-3）
  - `load_metric_api_data` / `load_category_api_data` 返回可序列化数据（TODO 8-4）
- 测试文件：
  - `tests/test_health.py` —— /health 存活检查
  - `tests/test_api_metrics.py` —— 未登录拦截 + 登录后指标返回
  - `tests/test_api_categories.py` —— 默认全部 / Fashion 筛选 / Grocery 筛选
  - `tests/test_error_handling.py` —— 400 错误 JSON 结构
- 尚未解决的问题：暂无
